...
function Onboarding({ onDone }) {
  ...
  const finish = () => { const u = { ...data, joinDate: new Date().toLocaleDateString("en-IN") }; safeSet("glUser", JSON.stringify(u)); onDone(u); };
  ...
  return (
    <div className="ob-wrap">
      ...
      <div className="ob-luffy-wrap">
        <img src="https://upload.wikimedia.org/wikipedia/en/1/1f/Monkey_D_Luffy_anime_pre_timeskip_design.png" alt="Luffy" className="ob-luffy-img" onError={e=>{e.target.style.display="none";}} />
...
function Block({ item, idx, open, onToggle }) {
  return (
    <div className={`block${open?" block-open":""}`} style={{"--bc":item.color, animationDelay:`${idx*55}ms`}} onClick={()=>onToggle(idx)}>
      ...
      <div className="block-detail"><p>{item.detail}</p></div>
    </div>
  );
}
    ...
    <>
      <div className={`app${ready?" app-ready":""}`}>
        ...
        <header className="hero">
          ...
          {[...Array(30)].map((_,i)=>(
            <div key={i} className="star" style={{width:i%5===0?3:2,height:i%5===0?3:2,top:`${(i*13+7)%72}%`,left:`${(i*17+3)%94}%`,animationDuration:`${2+(i%3)}s`,animationDelay:`${(i*0.25)%2}s`}}/>
          ))}
          <div className="hero-luffy-wrap">
            <div className="hero-luffy-halo"/>
            <img src="https://upload.wikimedia.org/wikipedia/en/1/1f/Monkey_D_Luffy_anime_pre_timeskip_design.png" alt="Luffy" className="hero-luffy" onError={e=>{e.target.style.display="none";}}/>
          </div>
        ...
        <nav className="nav">
          {[["today","📅 Today"],["code","⚔️ Code"],["crew","🏴‍☠️ Crew"]].map(([k,l])=>(
            <button key={k} className={`nav-btn${tab===k?" nav-active":""}`} onClick={()=>{setTab(k);setExpanded(null);}}>{l}</button>
          ))}
        </nav>
        <main className="main">
          {tab==="today" && (
            <div className="section-anim">
              ...
              {schedule.map((item,i)=>(<Block key={i} item={item} idx={i} open={expanded===i} onToggle={toggle}/>))}
            </div>
          )}
          {tab==="code" && (
            <div className="section-anim">
              ...
              {RULES.map((r,i)=>(
                <div key={i} className="rule-card" style={{animationDelay:`${i*60}ms`}}>
                  ...
                  <div><div className="rule-title">{r.title}</div><div className="rule-body">{r.body}</div></div>
                </div>
              ))}
            </div>
          )}
